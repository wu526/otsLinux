...
{
    machbstart_t *mbsp = MBSPADR; // 1M 的内存地址
    machbstart_t_init(mbsp);

    init_chkcpu(mbsp); // chkcpmm.c 检查 CPU
    init_mem(mbsp);    // chkcpmm.c 获取内存数据结构信息
    ...
    init_krlinitstack(mbsp); // chkcpmm.c
    init_krlfile(mbsp);      // fs.c
    init_defutfont(mbsp);    // fs.c
    init_meme820(mbsp);      // chkcpmm.c
    init_bstartpages(mbsp);  // chkcpmm.c, 设置页表
    init_graph(mbsp);        // graph.c
...
void machbstart_t_init(machbstart_t *initp)
{
    ...
    initp->mb_migc = MBS_MIGC; // 等价于字符串: LMOSMBSP
    return;
}
