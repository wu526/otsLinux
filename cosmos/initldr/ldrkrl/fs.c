...
{
    // 在映像中查找相应的文件, 并复制到对应的地址, 并返回文件的大小, 这里查找 kernel.bin文件
    u64_t sz = r_file_to_padr(mbsp, IMGKRNL_PHYADR, "Cosmos.bin"); // 该文件是 hal/目录下 生成的
    if (0 == sz)
    {
        kerror("r_file_to_padr err");
    }
    // 放置完成后更新机器信息结构中的数据
    mbsp->mb_krlimgpadr = IMGKRNL_PHYADR; // 0x2000000
    mbsp->mb_krlsz = sz;
