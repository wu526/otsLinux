...
_start:
_16_mode:
    mov    bp, 0x20 ;; 指向 GDT 中的第4个描述符, 即 16位数据段描述符;
    mov    ds, bp
    mov    es, bp
    mov    ss, bp
    mov    ebp, cr0
...
[BITS 32]
_32bits_mode:
    mov bp, 0x10 ;; 重新设置保护模式下的段寄存器 0x10, 是 32位数据段描述符的索引
    mov ds, bp
    mov ss, bp
    mov esi, [PM32_EIP_OFF] ;; 取出保存的eip
    mov esp, [PM32_ESP_OFF] ;; 还原栈顶
    jmp esi ;; eip=esi 回到了 realadr_call_entry 函数中, 即跳转回原来的位置
...
int131errmsg: db     "int 13 read hdsk  error", 0
