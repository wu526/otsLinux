...
typedef struct s_mlosrddsc
{
    ...
    u64_t mdc_ldrcodenr; // 映像文件中二级引导器的 文件头描述符 的索引号
    u64_t mdc_fhdnr;     // 映像文件中 文件头描述符 有多少个
    u64_t mdc_filnr;     // 映像文件中 文件 有多少个
...
// 将字符串 LMOSMBSP 存入到一个 8 字节的 u64_t 中
...
{
    u64_t mb_migc;         // LMOSMBSP offset: 0
    u64_t mb_chksum;       // offset: 8
    u64_t mb_krlinitstack; // 内核栈地址  offset: 16
    u64_t mb_krlitstacksz; // 内核栈大小 offset: 24
    u64_t mb_imgpadr;      // 操作系统映像
    u64_t mb_imgsz;        // 操作系统映像大小
    u64_t mb_krlimgpadr;   // 操作系统字体地址
    u64_t mb_krlsz;        // 操作系统字体大小
    u64_t mb_krlvec;
    u64_t mb_krlrunmode;
...
} __attribute__((packed)) machbstart_t; // packed 取消字节对齐
...
// REGCALL =  __attribute__((regparm(3))), 让GCC使用(EAX, EDX, ECX)寄存器传递参数
