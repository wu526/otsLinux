...
#define _X	0x40	/* hex digit */ // 该比特用于16进制数字
#define _SP	0x80	/* hard space (0x20) */  //该比特用于空格字符 0x20

extern unsigned char _ctype[];  //字符特性数组(表), 定义各个字符对应上面的属性
extern char _ctmp;  // 一个临时字符变量
//下面是一些确定字符类型的宏
#define isalnum(c) ((_ctype+1)[c]&(_U|_L|_D)) //是字符或数字[A-Z], [a-z], [0-9]
...
#define isxdigit(c) ((_ctype+1)[c]&(_D|_X)) //16进制数字
//下面两个定义中, 宏参数使用了前缀unsigned, 因此c应该加括号, 即表示成(c). 因为在程序中c可能是一个复杂表达式. 例如: 如果参数是
//a + b, 若不加括号, 则在宏定义中变成了 (unsigned)a+b, 这显然不对, 加了括号就能正确表示成(unsigned)(a+b);
#define isascii(c) (((unsigned) c)<=0x7f)  //是ASCII字符
#define toascii(c) (((unsigned) c)&0x7f) //转换成ASCII字符
//以下两个宏定义中使用了一个临时变量_ctmp的原因: 宏定义中, 宏的参数只能被使用一次. 但对于多线程来说是不安全的.因为两个或多个线程可能
//在同一时刻使用这个公共临时变量, 因此从Linux2.2.x 版本开始更改为使用两个函数来取代这两个宏定义
