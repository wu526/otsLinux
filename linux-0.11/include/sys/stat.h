...
//st_mode值的符号名称; 文件类型:
#define S_IFMT  00170000 //文件类型(8机制)
...
//文件属性位: S_ISUID用于测试文件的set-user-id标志是否置位; 若该标志置位,则当执行该文件时,进程的有效用户id将被设置为该文件宿主的用户id
//S_ISGID则是针对组ID进行相同处理
