 ...
 * Handbook", Sanches and Canton. 大多数来自 Sanches和Canton编著的 IBM微型计算机:程序员手册 一书
 */
...
//下面是有个软盘控制器一些端口和符号的定义
/* Fd controller regs. S&C, about page 340 */ //软盘控制器FDC寄存器端口
#define FD_STATUS	0x3f4  //主状态寄存器端口
#define FD_DATA		0x3f5 //数据端口
#define FD_DOR		0x3f2		/* Digital Output Register */ //数字输出寄存器, 也叫数字控制寄存器
...
#define STATUS_BUSYMASK	0x0F		/* drive busy mask */ // 驱动器忙位(每位对应一个驱动)
#define STATUS_BUSY	0x10		/* FDC busy */ //软盘控制器忙
#define STATUS_DMA	0x20		/* 0- DMA mode */ // 0-为DMA数据传输模式, 1:非DMA模式
#define STATUS_DIR	0x40		/* 0- cpu->fdc */ //传输方向: 0:cpu->fdc; 1:相反
#define STATUS_READY	0x80		/* Data reg ready */ //数据寄存器就绪位

/* Bits of FD_ST0 */ //状态字节0(ST0)各bit位的含义
...
#define ST0_INTR	0xC0		/* Interrupt code mask */ //中断代码位(中断原因); 00:命令正常结束; 01命令异常结束; 10命令无效; 11: FDD就绪状态改变

/* Bits of FD_ST1 */ //状态字节1(ST1)各比特位含义
#define ST1_MAM		0x01		/* Missing Address Mark */ //未找到地址标志(ID AM)
#define ST1_WP		0x02		/* Write Protect */ //写保护
#define ST1_ND		0x04		/* No Data - unreadable */ //未找到指定扇区
...
/* Bits of FD_ST2 */ //状态字节2(ST1)各比特位的含义
...
#define ST2_SEH		0x08		/* Scan Equal Hit */ //检索条件不满足
...
/* Bits of FD_ST3 */ //状态字节3(ST3)各bit位含义
#define ST3_HA		0x04		/* Head (Address) */ //磁头号
#define ST3_TZ		0x10		/* Track Zero signal (1=track 0) */ //0磁道信号
#define ST3_WP		0x40		/* Write Protect */ //写保护

/* Values for FD_COMMAND */ //软盘命令码
#define FD_RECALIBRATE	0x07		/* move to track 0 */ //重新校正(磁头退到0磁道)
#define FD_SEEK		0x0F		/* seek track */ //磁头寻道
#define FD_READ		0xE6		/* read with MT, MFM, SKip deleted */ //读数据(MT多磁道,MFM格式跳过删除数据)
